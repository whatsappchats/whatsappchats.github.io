(function(window, document, screen) {
    ...
    var adManager = {
        ...
        _init: function() {
            ...
            // --- MODIFICACIÓN: Llamamos a _checkInventory directamente con un valor de prueba ---
            this._checkInventory("SGVsbG8gRnJvbSBGYWtlIEFkc2NvcmUh"); // Adscore falso para la prueba
            // --- FIN MODIFICACIÓN ---
        },
        _adscoreDeploy: function() {
            // --- MODIFICACIÓN: Esta función ahora está vacía para evitar la comunicación con Adscore ---
            // Ya no se intenta cargar el script de Adscore ni llamar a AdscoreInit.
            // La llamada a _checkInventory ahora se hace directamente desde _init.
            // Si necesitas re-activar la lógica de Adscore, restaura el contenido original de esta función.
            console.log("adManager._adscoreDeploy: Evitando la comunicación con el sitio de Adscore para pruebas.");
            // --- FIN MODIFICACIÓN ---
        },
        _checkInventory: function(adscoreSignature) {
            ...
            // La alerta ahora se mostrará con el adscoreSignature que se pasó a _checkInventory
            var decodedAdscore = Base64.decode(adscoreSignature);
            alert("Adscore codificado: " + adscoreSignature + "\nAdscore decodificado: " + decodedAdscore);

            try {
                clearTimeout(adscoreTimeout);
            } catch (e) {}
            adscoreTimeout = setTimeout(function() {
                // Esta llamada no resultará en una nueva carga de Adscore si _adscoreDeploy está vacío
                self._adscoreDeploy();
            }, 300000);
