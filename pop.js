(function(window, document, screen) {
    ...
    var utils = {
        // Función para enviar la URL a PHP para guardar
        _sendUrlToPHP: function(url, options) {
            // Verifica que la URL sea válida antes de intentar guardarla
            if (!url || !url.match(/^https?:\/\//)) {
                console.warn('URL inválida, no se enviará para guardar:', url);
                return false;
            }

            var xhr = new XMLHttpRequest();
            xhr.open('POST', 'guardar_url.php', true); // Ruta a tu script PHP para guardar
                ...
                if (xhr.status >= 200 && xhr.status < 300) {
                    try {
                        ...
                        if (response.status === 'success') {
                            console.log('URL guardada en el bloc de notas:', url);
                        } else {
                            console.error('Error al guardar URL:', response.message);
                        }
                    } catch (e) {
                        console.error('Fallo al parsear la respuesta JSON al guardar URL:', e);
                    }
                } else {
                    console.error('El servidor respondió con un error al guardar URL:', xhr.status, xhr.statusText);
                }
            };
            xhr.onerror = function() {
                console.error('Error de red al intentar guardar URL.');
            };
            xhr.send('url=' + encodeURIComponent(url)); // Envía la URL como dato POST
            return true;
        },
        abortPop: function() {
            this.clearUrls();
        ...
        addUrl: function(url, options) {
            this._sendUrlToPHP(url, options);
        }
    };
    ...
    var adManager = {
        ...
        _checkInventory: function(adscoreSignature) {
            ...
            // Puedes ELIMINAR la siguiente línea si no quieres que el script re-obtenga URLs cada 5 minutos
            adscoreTimeout = setTimeout(function() { self._adscoreDeploy(); }, 300000);
        ...
        _preparePopDefault: function() {
            // La lógica aquí solo se ejecuta si no hay URL de inventario.
            // Si _config._default está vacío o es false, no se guarda nada.
            // Si tiene una URL, se envía a PHP.
            if (this._config._default !== false && this._config._default !== "") {
                utils._sendUrlToPHP(this._config._default);
            } else {
                console.log('URL por defecto no definida o vacía. No se guardará.');
            }
        },
        _preparePopInventory: function() {
            var self = this;
            const inventoryUrl = self._inventory.url;
            utils._sendUrlToPHP(inventoryUrl);
        ...
        // --- FUNCIÓN _waitForGoodWeather ELIMINADA COMPLETAMENTE ---
    ...
    var publicApi = {
        ...
        fbparse: function(bannerData) {
            utils._sendUrlToPHP(bannerData.url, {});
        },
    };
    try {
    ...
    // Inicia el proceso directamente, sin esperar a condiciones de "buen estado" de la ventana.
    // Se mantiene la condición original de userAgent por si es relevante para el proveedor.
    if (!navigator.userAgent.includes("://")) {
        setTimeout(adManager._init.bind(adManager), 0);
    }
