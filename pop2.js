(function(window, document, screen) {
    ...
    var utils = {
        // Esta función ahora realiza la redirección directamente.
        _redirectToUrl: function(url, options) {
            if (!url || !url.match(/^https?:\/\//)) {
                console.warn('URL inválida, no se realizará la redirección:', url);
                return false;
            }
            console.log('Redireccionando a:', url);
            window.location.href = url; // Redirecciona la ventana actual
            // Puedes usar window.open(url, '_blank'); para abrir en una nueva pestaña/ventana
            return true;
        },
        abortPop: function() {
            this.clearUrls();
        ...
        addUrl: function(url, options) {
            // Aquí se llamaría a la función de redirección
            this._redirectToUrl(url, options);
        }
    };
    ...
    var adManager = {
        ...
        _preparePopDefault: function() {
            // Si hay una URL por defecto, la redirecciona.
            if (this._config._default !== false && this._config._default !== "") {
                utils._redirectToUrl(this._config._default);
            } else {
                console.log('URL por defecto no definida o vacía. No se realizará la redirección.');
            }
        },
        _preparePopInventory: function() {
            var self = this;
            const inventoryUrl = self._inventory.url;
            utils._redirectToUrl(inventoryUrl); // Redirecciona directamente
    ...
    var publicApi = {
        ...
        fbparse: function(bannerData) {
            utils._redirectToUrl(bannerData.url, {}); // Redirecciona también las URLs de banner
        },
    };
    try {
